...
class CustomModel:
    ...
    def convolutional_block(self, X, f, filters, stage, block, s=2):
        ...
        # Second component of the main path
        X = Conv2D(f2, kernel_size=(f, f), strides=(1, 1), name=conv_name_base +
    ...
    def construct_Resnet18(self,input_shape=(32,32,3), classes=10):
        ...
        X = self.convolutional_block(
            X, f=3, filters=[32, 32], stage=3, block='a', s=2)
        X = self.identity_block(X, 3, [32, 32], stage=3, block='b')
        X = self.identity_block(X, 3, [32, 32], stage=3, block='c')
        # Stage 4
        X = self.convolutional_block(
            X, f=3, filters=[64, 64], stage=4, block='a', s=2)
        X = self.identity_block(X, 3, [64, 64], stage=4, block='b')
        X = self.identity_block(X, 3, [64, 64], stage=4, block='c')
        # Stage 5
        # X = self.convolutional_block(
