...
class CustomModel:
    ...
    def convolutional_block(self,X, f, filters, stage, block, s=2):
        ...
        # First component of the main path
        X = Conv2D(f1, kernel_size=(f,f),strides=(s,s),name=conv_name_base + '2a',padding='same',kernel_initializer=glorot_uniform(seed=0))(X)
        X = BatchNormalization(axis=3, name=bn_name_base + '2a')(X)
        X = Activation('relu')(X)

        # Second component of the main path
        X = Conv2D(f2, kernel_size=(1,1),strides=(1,1),name=conv_name_base + '2b',padding='valid',kernel_initializer=glorot_uniform(seed=0))(X)
        X = BatchNormalization(axis=3, name=bn_name_base + '2b')(X)


        # shortcut path
        X_Shortcut = Conv2D(f2, kernel_size=(1,1),strides=(s,s),name=conv_name_base + '1',padding='valid',kernel_initializer=glorot_uniform(seed=0))(X_Shortcut)
    ...
    def construct_Resnet18(self,input_shape=(32,32,3), classes=10):
        ...
        # Stage 2
        X = self.convolutional_block(X, f=3, filters=[64,64], stage=2, block='a', s=1)
        X = self.identity_block(X, 3,[64,64], stage=2, block='b')
        # Stage 3
        X = self.convolutional_block(X, f=3, filters=[64,128], stage=3, block='a', s=2)
        X = self.identity_block(X,3,[64,128], stage=3, block='b')
        # Stage 4
        X = self.convolutional_block(X, f=3, filters=[128,256], stage=4, block='a', s=2)
        X = self.identity_block(X,3,[128,256], stage=4, block='b')
        # Stage 5
        X = self.convolutional_block(X, f=3, filters=[256,512], stage=5, block='a', s=2)
